...
#include <glob.hh>
class statistics{
	public:	typedef std::map<uint64_t,std::tuple<std::vector<uint32_t>,int>> istat_t;
				static std::map<std::string,double> stats(const istat_t &_alleles){
					std::map<std::string,double> stats;
					double N=std::accumulate(_alleles.begin(),_alleles.end(),0.0,[](double value,const istat_t::value_type& a){return(value+double(std::get<1>(a.second)));});
